...
Level.prototype.obstacleAt = function (pos, size) {
  var xStart = Math.floor(pos.x); //4.99999 = 4 
  var xEnd = Math.ceil(pos.x + size.x);
  var yStart = Math.floor(pos.y); //4.99999 = 4 
  var yEnd = Math.ceil(pos.y + size.y);

  //Consider edges of level space as walls
  if (xStart < 0 || xEnd > this.width || yStart < 0 || yEnd > this.height)
    return "wall";

  //Check each grid position starting at xStart, yStart 
...
Player.prototype.moveY = function(step, level, keys) {
  //Always accelerate the player downwards
  ...
  var obstacle = level.obstacleAt(newPos, this.size);
   ...
   //The Floor is also an obstacle - only allow jumps if on an obstacle
  if (obstacle)
  {
